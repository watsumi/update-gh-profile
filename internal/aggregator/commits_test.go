...
func TestAggregateCommitHistory(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Aggregate commits from multiple repositories on same date",
			commitHistories: map[string]map[string]int{
				"repo1": {
					"2024-01-01": 5,
				},
...
func TestAggregateCommitTimeDistribution(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Aggregate commits from multiple repositories at same time slot",
			timeDistributions: map[string]map[int]int{
				"repo1": {
					9: 5,
				},
		...
		{
			name: "Skip time slots out of range",
			timeDistributions: map[string]map[int]int{
				"repo1": {
					9:  5,
					25: 10, // Out of range (skipped)
					-1: 3,  // Out of range (skipped)
				},
			},
			wantHours: 1, // Only 9:00
			wantTotal: 5,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Verify time slots are within 0-23 range
...
func TestSortCommitTimeDistributionByHour(t *testing.T) {
	...
	// Verify sorted by time slot (ascending)
